defmodule Cympho.QualityGatesTest do
  ...
  describe "ExecutionState helpers" do
    test "require_different_actor? returns true when flag is set" do
      state = %{
        ...
        current_participant: "rev1",
        return_assignee: "exec1",
      ...
      policy = %Cympho.ExecutionPolicies.ExecutionPolicy{
        stage_configs: [
          %{"type" => "executor"},
          %{"type" => "reviewer", "require_different_actor" => true}
        ]
      }
    ...
    test "require_different_actor? returns false when not set" do
      ...
      policy = %Cympho.ExecutionPolicies.ExecutionPolicy{
        stage_configs: [%{"type" => "executor"}, %{"type" => "reviewer"}]
      }
    ...
    test "require_human? returns true when flag is set" do
      ...
      policy = %Cympho.ExecutionPolicies.ExecutionPolicy{
        stage_configs: [%{"type" => "executor"}, %{"type" => "reviewer", "require_human" => true}]
      }
    ...
    test "original_executor returns first history participant" do
      assert ExecutionState.original_executor(%{
               history: [%{participant: "exec1"}],
               current_participant: "rev1",
               return_assignee: "exec1"
             }) == "exec1"
    end
    ...
    test "original_executor falls back to current_participant when no history" do
      assert ExecutionState.original_executor(%{history: [], current_participant: "rev1"}) ==
               "rev1"
    end
  end
  ...
  describe "execution_policy_decision quality gates" do
    ...
    test "rejects decision when execution state is inactive" do
      result =
        Issues.execution_policy_decision(
          %Cympho.Issues.Issue{execution_state: nil},
          :approve,
          "someone"
        )
  ...
  describe "transition bypasses role check for execution policy issues" do
    ...
    test "engineer can transition to in_review with execution policy", %{
      issue: issue,
      executor: executor
    } do
