defmodule Cympho.GithubWebhookTest do
  ...
  describe "verify_signature/3" do
    setup do
      ...
      payload =
        "{\"action\":\"opened\",\"pull_request\":{\"html_url\":\"https://github.com/owner/repo/pull/123\"}}"
    ...
    test "returns :ok for valid signature", %{
      payload: payload,
      secret: secret,
    ...
    test "returns {:error, :unauthorized} for invalid signature", %{
      payload: payload,
      secret: secret
    } do
      invalid_signature = "sha256=invalidsignature"

      assert GithubWebhook.verify_signature(payload, invalid_signature, secret) ==
               {:error, :unauthorized}
    end
    ...
    test "returns {:error, :unauthorized} for empty string signature", %{
      payload: payload,
      secret: secret
    } do
    ...
    test "returns {:error, :unauthorized} for nil secret", %{
      payload: payload,
      valid_signature: signature
    } do
    ...
    test "returns {:error, :unauthorized} for empty string secret", %{
      payload: payload,
      valid_signature: signature
    } do
    ...
    test "returns {:error, :unauthorized} when signature format is wrong", %{
      payload: payload,
      secret: secret
    } do
      ...
      assert GithubWebhook.verify_signature(payload, wrong_format, secret) ==
               {:error, :unauthorized}
    end
    ...
    test "handles different payloads correctly" do
      ...
      payload1 =
        ~s({"action":"opened","pull_request":{"html_url":"https://github.com/owner/repo/pull/1"}})

      payload2 =
        ~s({"action":"closed","pull_request":{"html_url":"https://github.com/owner/repo/pull/2"}})
