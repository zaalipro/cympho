defmodule Cympho.SubissueCompletionTest do
  ...
  setup do
    {:ok, company} = Cympho.Companies.create_company(%{name: "Test Co", slug: "sub-#{abs(System.unique_integer())}"})
  ...
  describe "subissue auto-completion" do
    test "parent auto-completes when all children are done", %{project: project} do
      {:ok, parent} = Issues.create_issue(%{title: "Parent", project_id: project.id, status: :in_progress})
      {:ok, child1} = Issues.create_issue(%{title: "C1", project_id: project.id, parent_id: parent.id, status: :in_progress})
      {:ok, child2} = Issues.create_issue(%{title: "C2", project_id: project.id, parent_id: parent.id, status: :in_progress})
    ...
    test "parent without children completing does not crash", %{project: project} do
      {:ok, issue} = Issues.create_issue(%{title: "Solo", project_id: project.id, status: :in_progress})
    ...
    test "parent does not complete if some children are still active", %{project: project} do
      {:ok, parent} = Issues.create_issue(%{title: "Parent", project_id: project.id, status: :in_progress})
      {:ok, child1} = Issues.create_issue(%{title: "C1", project_id: project.id, parent_id: parent.id, status: :in_progress})
      {:ok, _child2} = Issues.create_issue(%{title: "C2", project_id: project.id, parent_id: parent.id, status: :in_progress})
  ...
  describe "issue-goal linking" do
    test "issue can reference a goal", %{project: project} do
      ...
      {:ok, issue} = Issues.create_issue(%{title: "Task", project_id: project.id, goal_id: goal.id})
