defmodule Cympho.PaginationTest do
  ...
  defp company(name, slug, inserted_at) do
    Repo.insert!(%Company{
      name: name,
      slug: slug,
  ...
  defp collect_ids(opts) do
    do_collect(opts, nil, [])
  end

  defp do_collect(opts, cursor, acc) do
    page = Pagination.page(Company, Keyword.put(opts, :after, cursor))
    acc = acc ++ Enum.map(page.entries, & &1.id)

    if page.has_more?, do: do_collect(opts, page.next_cursor, acc), else: acc
  end

  describe "page/2 has_more?/cursor bookkeeping" do
    test "a full page reports has_more? with a non-nil cursor" do
      for i <- 1..5, do: company("Co #{i}", "co-#{i}", at(i))

      page = Pagination.page(Company, limit: 2)
    ...
    test "a partial final page reports has_more? false and a nil cursor" do
      for i <- 1..3, do: company("Co #{i}", "co-#{i}", at(i))

      page = Pagination.page(Company, limit: 10)
  ...
  describe "page/2 keyset correctness" do
    ...
    test "ascending cursor fields paginate in ascending order" do
      ...
      expected =
        Repo.all(from c in Company, order_by: [asc: c.name, asc: c.id], select: c.id)

      assert collect_ids(limit: 2, cursor_fields: [{:name, :asc}, {:id, :asc}]) == expected
    end
    ...
    test "mixed-direction cursor fields stay contiguous" do
      ...
      for i <- 1..6, do: company("Co #{i}", "co-#{i}", ts)

      expected =
        Repo.all(from c in Company, order_by: [desc: c.inserted_at, asc: c.id], select: c.id)

      got = collect_ids(limit: 2, cursor_fields: [{:inserted_at, :desc}, {:id, :asc}])
