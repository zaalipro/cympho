defmodule CymphoWeb.KanbanLiveTest do
  ...
  setup do
    {:ok, issue_backlog} = Issues.create_issue(%{title: "Backlog Issue", description: "backlog", status: :backlog, priority: :high})
    {:ok, issue_todo} = Issues.create_issue(%{title: "Todo Issue", description: "todo", status: :todo, priority: :medium})
    %{issue_backlog: issue_backlog, issue_todo: issue_todo}
  end
  ...
  describe "Kanban rendering" do
    ...
    test "renders data-issue-id on cards" do
      {:ok, _view, html} = live(conn(), "/kanban")
