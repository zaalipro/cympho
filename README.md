...
| Pause, resume, and stop | Publicly documents agent pause/resume/terminate; public issues show demand for stronger in-flight kill behavior. | Global Pause/Resume/Stop controls pause the company, release active issues, cancel run rows, and mark agents paused. Adapter-level subprocess cancellation still needs hardening before this is a full kill switch. | **No clear winner yet.** Cympho now has a better control-plane surface, but should not overclaim process-level termination. |
...
| Tool-call tracing and audit | Public README claims full tool-call tracing and immutable audit logging. | ToolCallTraces, activities, governance audit logs, event replay, and issue-level execution briefs. | **Tie.** Cympho should not imply Paperclip lacks tracing. |
...
| Cost controls | Monthly budgets, hard stops, token/cost tracking, and budget-aware governance are core public claims. | Budgets, hard stops, runtime capacity, provider/model runtime profiles, cost posture, and swarm runtime-mix selection. | **Tie.** Cympho adds swarm mix controls; Paperclip explains cost controls more clearly publicly. |
...
| --- | --- | --- |
| No reliable emergency stop for in-flight agent work | [paperclipai/paperclip#2224](https://github.com/paperclipai/paperclip/issues/2224) describes no kill switch for active runs, status desync after OS kills, and concurrent process confusion. [#1158](https://github.com/paperclipai/paperclip/issues/1158), [#3722](https://github.com/paperclipai/paperclip/issues/3722), and [#4266](https://github.com/paperclipai/paperclip/issues/4266) describe hanging or stuck running heartbeats that require manual intervention. | **Partly implemented:** global shell Pause/Resume/Stop controls pause the company, stop active Cympho orchestrator sessions, release in-progress issue ownership, cancel active run rows, and mark agents paused. Full adapter-level subprocess termination is still a hardening target. |
| Agents miss attached context | [#2536](https://github.com/paperclipai/paperclip/issues/2536) reports files attached to an issue not appearing in heartbeat context. | Cympho already has issue attachments, work products, issue digest, and owner-readable evidence. Keep tightening agent prompt context so attached artifacts are always visible to the right runtime. |
| Instructions can be silently dropped or hard to edit | [#3833](https://github.com/paperclipai/paperclip/issues/3833) reports AGENTS.md not injected for a local adapter; [#3552](https://github.com/paperclipai/paperclip/issues/3552) and [#2068](https://github.com/paperclipai/paperclip/issues/2068) report instructions UI save/display failures. | Cympho's Instruction Studio, instruction files, prompt contracts, and adapter readiness checks should be treated as premium surface area, with tests that prove each adapter receives the intended instructions. |
| Heartbeat context can be too expensive | [#906](https://github.com/paperclipai/paperclip/issues/906) asks for role-tiered heartbeat skills to avoid loading a large universal protocol on every run; [#3794](https://github.com/paperclipai/paperclip/issues/3794) asks for context budget telemetry. | Cympho should keep role-specific prompts, runtime profiles, spend posture, and future context-budget telemetry focused on reducing waste per heartbeat and swarm worker. |
| Workspace paths can surprise external runtimes | [#1425](https://github.com/paperclipai/paperclip/issues/1425), [#1841](https://github.com/paperclipai/paperclip/issues/1841), and [#3422](https://github.com/paperclipai/paperclip/issues/3422) show friction around workspace path ownership, fallback paths, and integration with existing agent workspaces. | Cympho's workspace, environment, lease, preview, and adapter runtime configuration should keep pushing toward explicit workspace ownership and visible recovery actions. |
