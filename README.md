...
- **Operations console**: monitor runtime mode, agent capacity, adapter health, prompt readiness, blocked work, review nudges, and execution risk from one place. Named resource profiles enforce one node-wide total-run gate plus a sub-limit for local CLI processes; production also checks measured host/cgroup headroom before starting another local process, while gateway work uses the total gate without consuming the local-process sub-limit.
...
Paperclip is the stronger public benchmark today: larger public footprint, better public docs/community surface, stronger mobile/product story, and a much larger prebuilt company catalog. Cympho is younger, rougher, and less battle-proven, but it is optimizing for supervised operations, owner-readable evidence, and CTO-mediated swarm work. This table is a selected source comparison, not a latest-feature parity certificate; the maintained gap register appears in [`paperclip_gap.md`](paperclip_gap.md).

Source basis: the latest upstream audit inspected Paperclip public `master` at `821573ede850441d5043ecd4860ee70a2a0374b1` (commit date 2026-08-25) and stable `v2026.824.1` at `8e6edcdfa911151adba26be49a41cf5017b3aade`. Historical rows originated against the 2026-07-30 baseline and are retained only as narrow Cympho regression claims. The Cympho side is based on this repository, current screenshots, `paperclip_gap.md`, and `mix cympho.compare`; none of these is a side-by-side production, scale, RAM, or CPU benchmark.
...
| Idle heartbeat cost | Historical public reports described timer heartbeats waking agents with no assigned work and archived companies consuming limits. | Dispatcher-delegated agents are event-driven and timer-free while idle. A matched 100-agent development workload observed 0 idle queries and 0 timers versus 800 queries and 100 timers on the legacy direct path; raw artifacts are in `benchmarks/results/`. | **Specific Cympho improvement verified, cross-product scale unproven.** This does not replace a matched low-VPS Paperclip benchmark. |
...
This comparison is intentionally not a trophy wall. Paperclip is the more mature public product and has the stronger ecosystem story today. Cympho has sharper operational ideas in a few places, but it is younger and should be judged by what is implemented and verified here. The latest source audit inspected Paperclip public `master` at `821573ede850441d5043ecd4860ee70a2a0374b1` (commit date 2026-08-25) and stable `v2026.824.1` at `8e6edcdfa911151adba26be49a41cf5017b3aade`. Many narrow rows below preserve historical issue research from the earlier 2026-07-30 baseline; they are not evidence that the issue still exists upstream. The table does not claim exhaustive latest parity, side-by-side runtime quality, complete physical-device coverage, or production-load/resource benchmarking. Current gaps and verification criteria live in [`paperclip_gap.md`](paperclip_gap.md).
...
| Remote sandbox execution | Environment-driver plugins provision and operate remote sandboxes across multiple providers. | The `ssh` driver runs workspaces on any reachable host using OTP's `:ssh` — acquire/execute/release/cancel with pinned host keys, in-memory keys, and credentials from the company secret store. Vendor SaaS drivers (E2B/Daytona/Modal) stay unregistered and fail closed. | **Tie, different shape.** Paperclip covers more managed vendors; Cympho needs no vendor account and its suite runs against a live SSH daemon rather than a fake. |
...
| Evaluation and feedback | Saved skill-test runs and feedback exports connect outcomes back to agent context. | Company-scoped evaluation suites, immutable runs with provenance, redacted result traces, deterministic reruns, run comparisons, and owner feedback votes, plus Instruction Studio additive patch previews with durable rollback. | **Tie on repository behavior.** Cympho now has durable evaluation records; Paperclip still has the stronger public feedback-export product story. |
...
| External AI control | Publicly documents an MCP Tool Gateway and Apps with governed tool access. | Built-in MCP server exposes company-scoped static tools plus dynamically registered tools gated by fail-closed per-agent grants, revocation, and MCP mutation rate limits. | **Mixed.** Paperclip has the broader public gateway/apps story; Cympho now has direct control-plane MCP with governed dynamic registration. |
...
| Selective portability | Portable company packages support selective content and standard local or repository-backed sources. | Read-only import preview, selective export includes, JSON/path/directory/GitHub sources, and merge into an existing company with skip/replace/rename/fail writers. Repository sources require a pinned commit SHA. Merge covers labels, projects, goals, and agents; identity and issues stay whole-company-import only. | **Tie, different shape.** Comparable breadth, with Cympho stricter on previewing, ref pinning, and refusing to merge identity by name. |
...
mix cympho.compare --json    # machine-readable selected-claim report
mix cympho.compare --strict  # exit non-zero when a selected local check is open
```

The task introspects Cympho's live OTP tree, registered adapter list, and exported context functions for a selected claim set that originated against the 2026-07-30 baseline. Its metadata records the latest audited Paperclip master and stable commits, but the command does not enumerate the complete latest upstream surface. A zero selected-gap count means only that those local checks did not regress; it is not feature-parity or low-resource proof. Use [`paperclip_gap.md`](paperclip_gap.md) for the current upstream gap register.
