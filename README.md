...
| Sidebar and inbox trust | Public issues and discussions report stale inbox/sidebar badge counts after read, resolve, or dismiss actions. | Server-owned company unread counts feed the sidebar, and LiveView receives company PubSub badge updates after create/read/dismiss/archive/restore/bulk-read actions. | **Cympho wins on this tested path.** Paperclip may improve here; this is not a whole-UI reliability benchmark. |
| Human action queue | Public issues ask for a clean board-user queue for work assigned to humans instead of noisy touched/read notification inboxes. | Inbox has a `Needs my action` lane backed by non-terminal issues assigned to the current user. | **Cympho wins on this narrow queue.** Broader human approval UX still needs real usage polish. |
...
| Mobile and polish | README explicitly positions mobile management and has a more mature product story. | Operational UI is dense and improving, but mobile polish is not yet the strongest proof point. | **Paperclip wins.** Cympho should earn this with real mobile QA. |
...
| Cost and governance | Monthly budgets, hard stops, token/cost tracking, approvals, and governance are central public claims. | Budgets, runtime capacity, review mode, governance gates, runtime profiles, and swarm model/effort mix control. | **Tie.** Cympho has sharper swarm cost controls; Paperclip explains the baseline more clearly. |
...
| Mobile and public polish | README explicitly markets managing autonomous businesses from a phone. | LiveView UI is operationally dense, but mobile polish is not yet the product's strongest proof point. | **Paperclip.** Cympho should earn this with real mobile QA, not claims. |
...
| Inbox/sidebar count reliability | Public Paperclip issue/discussion signals include stale badge counts after read/resolved/dismissed inbox items. | `Cympho.Inbox.unread_count_for_company/1` owns the count, sidebar LiveViews subscribe to company badge PubSub, and mutations publish updated counts. | **Cympho on this narrow path.** This is a code-backed Cympho claim, not a broad claim that Paperclip's UI is unreliable everywhere. |
| Human action inbox | [paperclipai/paperclip#3256](https://github.com/paperclipai/paperclip/issues/3256) asks for a "Needs my action" board-user view filtered by `assigneeUserId`, and [#923](https://github.com/paperclipai/paperclip/issues/923) argues that human blockers need issue-backed tasks, not noisy inbox notifications. | Cympho's Inbox now has a `Needs my action` lane, count card, and action-queue shortcut backed by open issues assigned to the current human user through `assignee_user_id`. | **Cympho on this focused operator queue.** This does not claim full board-workflow superiority; it makes human blockers first-class and filterable. |
...
| External AI control | Focuses on coordinating agents through Paperclip's app and operational workflows; MCP is not documented in the public README. | Built-in MCP server exposes Cympho tools to external AI clients. | **Cympho.** Useful when another AI client should drive the company OS as tools. |
| Cost controls | Monthly budgets, hard stops, token/cost tracking, and budget-aware governance are core public claims. | Budgets, hard stops, runtime capacity, provider/model runtime profiles, cost posture, unpriced-usage warnings, and swarm runtime-mix selection. | **Tie.** Cympho adds sharper unpriced/spend warnings and swarm mix controls; Paperclip explains cost controls more clearly publicly. |
