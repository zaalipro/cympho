...
| Remote sandbox execution | Environment-driver plugins provision and operate remote sandboxes across multiple providers. | EnvironmentDriver contract, registry, and Fake acquire/execute/release exist for local lifecycle tests; no real remote provider (E2B/Daytona/etc.) is registered. | **Paperclip.** A Fake driver is not remote provider execution. |
...
| Selective portability | Portable company packages support selective content and standard local or repository-backed sources. | Read-only import preview, selective export includes, and local JSON/path package sources exist; GitHub/ref sources and full merge collision writers (skip/replace/rename) remain open. | **Paperclip.** Cympho has safer previewing and partial selective export, not full package breadth. |
