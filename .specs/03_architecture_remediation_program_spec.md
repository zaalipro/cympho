...
> **Status (2026-05-29): deferred — recommended as individually-reviewed PRs.** The five decompositions (agent_actions, issue_digest, issues, agent_live/show, runtime_operations) are pure, behaviour-preserving refactors with very large diffs and zero functional change. The risk-per-value of doing all five in one autonomous pass is poor: a subtle visibility/compile-order slip can regress behaviour that the suite won't fully prove, and the diffs are hard to review in bulk. One of them (IssueLive.Show) is already owned by spec 02, and the spec itself calls for "one module per PR." Recommendation: tackle each as its own reviewed PR, following the spec-02 extraction pattern, after the shipped correctness work (PRs 1–4) has been reviewed/merged.
...
> **Status (2026-05-29):** TASK-056 done — `deploy_example.sh` `required_vars` and `CLAUDE.md` now document the fail-closed `CYMPHO_ENCRYPTION_KEY` / `CYMPHO_USER_JWT_SECRET` / `CYMPHO_AGENT_JWT_SECRET` requirement. TASK-057 done — `mix test` green (2165, 0 failures) after every PR. TASK-058 partial — `mix format` clean for all changed files (8 pre-existing unrelated files remain unformatted and were intentionally left); `mix credo`/`mix dialyzer` are informational/not-CI-enforced per CLAUDE.md and were not run as a gate. TASK-059 (manual two-company browser smoke) deferred — the company-isolation paths are covered by the PR-1/PR-4 integration tests.

- [x] **TASK-056** [docs] Document the required JWT secret env vars and the "unscoped `get_*(id)` is internal-only" convention. Paths: `deploy_example.sh`, `CLAUDE.md`. Implements: `REQ-002`, `DES-004`. Verifies: `N/A`. Depends: `TASK-020`, `TASK-011`. Done when: both files state the requirement/convention explicitly.
- [x] **TASK-057** [verification] Run the full test suite. Paths: `N/A — command`. Implements: `N/A`. Verifies: `TEST-018` covering `AC-028`, `AC-029`, `AC-031`, `AC-038`. Depends: `TASK-001`–`TASK-055`. Done when: `mix test` exits 0.
