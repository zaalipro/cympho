defmodule CymphoWeb.Components.CompanySwitcherStatic do
  ...
  def company_switcher_static(assigns) do
    ~H"""
    <div
      id="company-switcher-wrapper"
      data-companies={Jason.encode!(@companies)}
      data-current-company-id={@current_company_id}
    >
      <div
      ...
      >
        <div class="bg-panel border border-border rounded-panel shadow-dialog w-full max-w-lg mx-4 overflow-hidden">
          ...
          <div id="company-switcher-results" class="max-h-72 overflow-y-auto p-2">
            <div
              class="px-3 py-8 text-center text-text-secondary text-sm hidden"
              id="company-switcher-empty"
            >
              <svg
                class="w-8 h-8 mx-auto mb-2 text-text-quaternary"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
