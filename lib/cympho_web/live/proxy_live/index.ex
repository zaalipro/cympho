defmodule CymphoWeb.ProxyLive.Index do
  ...
  def handle_event("show_edit_form", %{"id" => id}, socket) do
    with true <- socket.assigns.proxy_admin?,
         {:ok, proxy} <- Proxies.get_company_proxy_profile(socket.assigns.company_id, id) do
      {:noreply,
       socket
       |> assign(:show_form, true)
  ...
  def handle_event("save", %{"proxy_profile" => params}, socket) do
    if socket.assigns.proxy_admin? do
  ...
  def handle_event("delete", %{"id" => id}, socket) do
    with true <- socket.assigns.proxy_admin?,
         {:ok, proxy} <- Proxies.get_company_proxy_profile(socket.assigns.company_id, id),
  ...
  def handle_event("test", %{"id" => id}, socket) do
    with true <- socket.assigns.proxy_admin?,
         {:ok, tested} <- Proxies.test_proxy_profile(socket.assigns.company_id, id) do
      flash =
  ...
  defp proxy_admin?(_socket), do: false
