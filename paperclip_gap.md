...
| L3b | P0 | **Residual safety pass delivered; dispatch/wake retry residual open** | **Durable bounded stranded-work recovery.** Company-scoped recovery cases retain redacted source snapshots, parent/root lineage, leases, an active-source uniqueness fence, and append-only attempt audit. Fingerprint version 2 adds only the bounded durable liveness CAS timestamp for each source: a running run's `last_heartbeat_at` (falling back to `inserted_at`), a pending/queued run's `inserted_at`, or an issue checkout's `checked_out_at` (falling back to `updated_at`); no arbitrary mutable timestamp is fingerprinted. Direct, due, and exhaustion paths fail closed unless the final locked source still matches the exact tenant, issue, source/run/agent identities, status, fingerprint/liveness token, and freshness requirements. Each due row is selected and reserved in one short `FOR UPDATE SKIP LOCKED LIMIT 1` transaction, with its callback after commit. The complete bounded recovery policy is persisted once, is immutable for the active lineage, and is inherited by retry children. Terminal recovery governance locks case → approval and atomically commits the denial, cancellation, or expiry outcome with linked case resolution; it does not lock or mutate the issue, which remains blocked from the earlier exhaustion transaction. Approved retry and exhaustion acquire the issue after case/approval when their mutation requires it. Audit, company events, recovery events, and OwnerAttention publish best-effort only after commit. Dispatcher crash cleanup handles exact pre-crash runs before an unbound checkout, permits a durably stale cohort, defers to a fresh/live successor, and completes conservative checkout follow-up before closing successful run recovery. Deferred attempts remain in append-only audit with stable ordinals while their returned retry-budget slot and backoff use the separate retained budget count. Operations removes orphan run IDs from the waiting-run pass so overlapping pending/queued sources are counted and recovered once (`lib/cympho/recovery.ex`; `lib/cympho/recovery/fingerprint.ex`; `lib/cympho/heartbeat_engine.ex`; `lib/cympho/heartbeat_engine/watchdog.ex`; `lib/cympho/orchestrator/dispatcher.ex`; `lib/cympho/board_approvals.ex`; `lib/cympho/runtime_operations.ex`; `test/cympho/recovery/fingerprint_test.exs`; `test/cympho/recovery_test.exs`; `test/cympho/board_approvals/recovery_action_test.exs`; `test/cympho/board_approvals/transition_race_test.exs`; `test/cympho/board_approvals/execution_claim_test.exs`; `test/cympho/heartbeat_engine/watchdog_test.exs`; `test/cympho/orchestrator/dispatcher_test.exs`; `test/cympho_web/live/operations_live_test.exs`; `test/cympho/owner_attention_test.exs`). Ordinary dispatch-failure and wake-delivery lineage, cluster-wide owner fencing, durable PubSub/outbox delivery, measured low-resource RAM/CPU/VPS evidence, managed workspace-service adoption, and full/latest Paperclip parity all remain open. | Deliver durable ordinary dispatch-failure and wake-delivery lineage plus the remaining documented exit evidence before closing L3b. Do not infer cluster-wide ownership, crash-proof notification delivery, low-resource performance, managed service adoption, or Paperclip parity from this source/test safety pass. |
...
- [ ] **L3b durable bounded stranded-work recovery — residual safety pass
  ...
  bounded liveness CAS timestamps; destructive paths revalidate exact locked
  tenant/source/freshness guards; due reservations are atomic, one-row
  `FOR UPDATE SKIP LOCKED` transactions; and each lineage retains its complete
  immutable policy. Terminal recovery governance locks case → approval and
  commits the approval outcome plus linked case resolution before best-effort
  notifications; it does not lock or mutate the already-blocked issue. Crash
  cleanup is exact-run-first,
  defers to fresh/live successors, handles stale cohorts conservatively, keeps
  attempt audit append-only with a separate retry budget, and deduplicates the
  Operations orphan/waiting overlap. Ordinary dispatch-failure and
  wake-delivery lineage, cluster-wide owner fencing, durable PubSub/outbox
  delivery, measured low-resource RAM/CPU/VPS evidence, managed
  workspace-service adoption, and full/latest Paperclip parity remain open.
