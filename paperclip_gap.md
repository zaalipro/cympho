...
Current Paperclip stable-tag audit: `v2026.824.1`, `8e6edcdfa911151adba26be49a41cf5017b3aade`
...
## Latest-upstream gap register (stable 2026.824 and current master)
...
`~/dev/research/paperclip`. Stable behavior is summarized from
`git show master:releases/v2026.824.0.md` and the candidate patch note from
`git show v2026.824.1:releases/v2026.824.1.md`. The Git topology matters:
`v2026.824.0` (`664052f8`) is the merge base and an ancestor of both refs, while
`v2026.824.1` is a divergent four-commit candidate branch, not an ancestor of
master. At the audited refs, `v2026.824.0..master` and
`v2026.824.1..master` each contain 187 commits, and
`master..v2026.824.1` contains four. Feature-flagged master surfaces are
evidence of implementation, not claims that every stable install enables them
by default.

| ID | Priority | Status | Gap and authoritative evidence | Exit evidence |
| --- | --- | --- | --- | --- |
| L1 | P0 | **In progress** | **Benchmark truth.** `mix cympho.benchmark_idle_agents` now provides a rollback-safe idle-fleet workload, and `benchmarks/results/` retains matched delegated/direct development artifacts. `mix cympho.compare` now rejects missing or malformed comparative manifests and structurally checks exact clean revisions, an explicit ≤2-core/2-GiB Linux cgroup-v2 slice, matched app/database limits and protocol hashes, every required idle/active/wake/restart cell, five matched repetitions per product/cell, declared resource/latency metrics, raw-file checksums, and zero declared correctness/OOM/recovery failures. The row still remains open even after that structural gate because raw samples, events, and database evidence are not yet schema-validated and recomputed into those declarations. No reproducible side-by-side low-VPS Paperclip workload exists yet; local structure checks do not prove RAM, CPU, latency, or production reliability. | A pinned workload reports app + Postgres + child-process RSS/CPU, DB query rate, throughput, p95 latency, and correctness for idle 100/500/1,000-agent fleets, bounded active-run levels, wake storms, and restart recovery on a documented low-resource VPS, with versioned raw schemas and recomputation that reconcile every published value. |
...
| L3 | P0 | **In progress; node-local gate delivered** | **Capacity admission.** Checkout serializes per-agent capacity in PostgreSQL. Named profiles now separately bound total sessions, local OS-process runs, Repo, and Finch pools. `Cympho.RuntimeAdmission` classifies the concrete resolved adapter, fails unknown adapters into the local lane, atomically grants monitored total/local slots, and in production rejects new local starts when effective host/cgroup headroom is at or below the configured floor or cannot be measured. Every built-in worker registers and adopts its claim before provider work. Worker-bound admission and bounded PID-tree cleanup cover normal completion, cooperative cancellation, controller death while the worker remains schedulable, and manager recovery. The shipped Alpine image now installs `tini` as PID 1, and Docker CI is wired to run a fail-closed orphan-adoption/reaping probe (`Dockerfile`; `scripts/assert-orphan-reaping.sh`; `scripts/docker-build-test.sh`; `test/cympho/container_init_reaping_test.exs`; `.github/workflows/ci.yml`). This follows upstream reaping commit `5db8ce3c44cf807b3d012fffe47b171f637dd1d2` (`Dockerfile`; `scripts/assert-orphan-reaping.sh`; `server/src/__tests__/container-init-reaping.test.ts`) and applies only to Cympho's container image: the native systemd service does not need a container init. Docker is unavailable on the audited local host, so only the CI wiring and static contract were inspected locally; no local behavioral image result is claimed. Neither deployment yet has an independent Port custodian or per-run cgroup/container: brutal adapter-worker death can release a node-local claim before OS descendants are confirmed gone, and descendants surviving the direct Port child remain a portability residual. Gateway adapters consume a total slot but not the local sub-limit. Dispatcher advisory checks avoid doomed checkouts, while the Orchestrator gate covers manual, heartbeat, retry, and fallback paths. Company Operations exposes only a tenant-neutral delay signal; doctor and bounded telemetry retain node-level posture for instance operators. Limits remain per node, not cluster-wide, and the low-VPS evidence still belongs to L1. | Obtain the Docker CI behavioral result, reproduce enforcement and gateway progress under L1's matched low-VPS workload, move Port ownership and the claim into a supervised per-session custodian, then add cgroup-v2 containment on supported Linux deployments. Do not claim crash-proof containment, per-process ceilings, or cluster-wide admission. |
| L3a | P0 | Open | **Managed workspace-service ownership and restart adoption.** Paperclip persists and validates enough local-service identity to reconcile a surviving listener/process group with its workspace after controller or server restart, and rejects mismatched ownership (`bd059a073dc6cbf2ac2f0295ecc5a0b8aab60e5a`; `2eb9a09c0c2882953020c2ef8e3f57c53d3a42bd`; `packages/shared/src/worktree-port-registry.ts`; `server/src/services/local-service-supervisor.ts`). Cympho's `workspace_runtime_services` rows record logical service and preview metadata but not the PID, process group, listener owner, or persisted workspace-process identity needed for safe adoption (`lib/cympho/workspaces/runtime_service.ex`). | Persist an unambiguous company/workspace/service/process identity; on restart, adopt only a live matching process group and listener in the expected workspace, reject collisions or stale rows, and prove stop/restart/recovery without spawning duplicates or signaling a foreign process. |
| L3b | P0 | **Residual safety pass delivered; dispatch/wake retry residual open** | **Durable bounded stranded-work recovery.** Company-scoped recovery cases retain redacted source snapshots, parent/root lineage, leases, an active-source uniqueness fence, append-only attempt audit, and the effective stale threshold used across asynchronous approval execution. Fingerprint version 2 adds only the bounded durable liveness CAS timestamp for each source: a running run's `last_heartbeat_at` (falling back to `inserted_at`), a pending/queued run's `inserted_at`, or an issue checkout's `checked_out_at` (falling back to `updated_at`); the separately persisted threshold and arbitrary mutable timestamps are not fingerprinted. Destructive direct, due, exhaustion, and approved-retry paths require the exact version-2 key set/map size plus locked tenant, issue, source/run/agent identities, authoritative status, fingerprint/liveness token, error family, durable age, and live-owner/successor fences. The canonical retained order is recovery case → board approval → issue → run; terminal run lifecycle uses issue → run, while start/heartbeat/metadata remain run-only. Recovery execute-once locks case → approval before inserting/checking its effect. One validated cutoff flows from Watchdog/Dispatcher/Operations selection through successor checks and final CAS. Running scans use `COALESCE(last_heartbeat_at, inserted_at) < cutoff`; equality and an otherwise exact young source defer the same lineage rather than superseding it. Each due row is selected and reserved in one short `FOR UPDATE SKIP LOCKED LIMIT 1` transaction, with its callback after commit. The complete bounded policy is immutable for the active lineage and inherited by retry children; exact partial repeats are resolved against the locked snapshot. A new fingerprint closes any claimed attempt as skipped, clears its lease, and links the child atomically. Heartbeat retry children retain the locked run status for due-kind selection. Terminal recovery governance locks case → approval and atomically commits denial, cancellation, or expiry with linked case resolution; it does not lock or mutate the issue, which remains blocked from exhaustion. Audit, events, dispatcher/workspace effects, and OwnerAttention remain post-transaction best effort. Dispatcher crash cleanup handles exact pre-crash runs before an unbound checkout, permits a durably stale cohort, defers to a fresh/live successor, and completes conservative checkout follow-up before closing successful run recovery. Operations removes orphan run IDs from the waiting-run pass (`lib/cympho/recovery.ex`; `lib/cympho/recovery/fingerprint.ex`; `lib/cympho/heartbeat_engine.ex`; `lib/cympho/heartbeat_engine/watchdog.ex`; `lib/cympho/orchestrator/dispatcher.ex`; `lib/cympho/board_approvals.ex`; `lib/cympho/runtime_operations.ex`; `test/cympho/recovery/fingerprint_test.exs`; `test/cympho/recovery_test.exs`; `test/cympho/board_approvals/recovery_action_test.exs`; `test/cympho/board_approvals/transition_race_test.exs`; `test/cympho/board_approvals/execution_claim_test.exs`; `test/cympho/heartbeat_engine/watchdog_test.exs`; `test/cympho/heartbeat_engine_test.exs`; `test/cympho/orchestrator/dispatcher_test.exs`; `test/cympho_web/live/operations_live_test.exs`; `test/cympho/owner_attention_test.exs`). Ordinary dispatch-failure and wake-delivery lineage, cluster-wide owner epochs/insertion fencing, durable PubSub/outbox delivery, measured low-resource RAM/CPU/VPS evidence, managed workspace-service adoption, and full/latest Paperclip parity remain open. | Deliver durable ordinary dispatch-failure and wake-delivery lineage plus the remaining documented exit evidence before closing L3b. Do not infer cluster-wide ownership, crash-proof notification delivery, low-resource performance, managed service adoption, or Paperclip parity from this source/test safety pass. |
...
| L5 | P1 | **In progress; readiness/read-only diagnostics foundation delivered** | **Install, service, and repair UX.** `Cympho.Diagnostics` and `mix cympho.doctor` retain their stable human/JSON, non-destructive source-checkout diagnostic contract. The current tranche adds a bounded, no-store `GET /api/health` readiness/build-identity contract for application, database, and packaged-migration compatibility, plus a read-only `cymphoctl` for version, exact-revision readiness, systemd status, and bounded logs (`lib/cympho/build_info.ex`; `lib/cympho/readiness.ex`; `lib/cympho_web/controllers/health_controller.ex`; `bin/cymphoctl`; `test/cympho_web/controllers/health_controller_test.exs`; `test/cymphoctl_script_test.exs`). Release building/deploy wiring carries the revision manifest and gates cutover on that exact revision with rollback re-probing (`deploy/build.Dockerfile`; `deploy.sh`; `test/cympho/release_operator_integration_test.exs`). The documented installer is now run from a cloned checkout; on a production rerun it validates and reuses an existing regular `.env`, refuses unsafe password replacement when configuration is missing or inconsistent, and is covered by focused source/shell behavior tests (`README.md`; `install.sh`; `test/cympho/install_safety_test.exs`). These are narrow rerun-safety and read-only service-operability foundations, not Paperclip's managed lifecycle. Paperclip stable/candidate still has broader install/onboard/update/service/repair flows (`cli/src/commands/`; `git show v2026.824.1:releases/v2026.824.1.md`). | Add versioned managed install, onboard, update, backup/restore, and repair workflows with interrupted recovery; preserve the delivered installer rerun guards, doctor, readiness/build-identity, read-only status/logs, exact-revision rollback, and secret-leak regression coverage. |
...
| L8 | P1 | Open | **Conversation and document review completeness.** Cympho has Simple thread, Plan/Ask interactions, document/revision schemas, review gates, and artifacts, but document creation does not establish a persisted current-revision identity and updates lack row-lock/CAS semantics. Paperclip additionally persists document annotation threads/comments/anchors and injects them into review context (`packages/db/src/schema/document_annotation_*`; `git show master:releases/v2026.824.0.md`). | Establish current revision identity and CAS updates first; then make live task conversation and anchored document/artifact feedback share one auditable thread, survive or explicitly remap across revision changes, reach the acting agent, and keep raw diagnostics out of Simple mode. |
...
- [ ] **L3 capacity admission — enforcement delivered, evidence in progress.**
  ...
  typed. The shipped container now uses `tini` as PID 1 and Docker CI is wired
  to probe orphan adoption/reaping; Docker is unavailable on the audited local
  host, so there is no local behavioral image result. Independent Port
  custody/OS containment and L1's matched low-VPS resource evidence remain open
  before this gap is closed.
- [ ] **L3a managed workspace-service ownership — open.** Persist and validate
  listener/process-group/workspace identity before adopting services across a
  restart.
- [ ] **L3b durable bounded stranded-work recovery — residual safety pass
  delivered; dispatch/wake retry residual open.** Fingerprint v2 uses only
  bounded liveness CAS timestamps; exact destructive guards follow retained
  case → approval → issue → run order (terminal run lifecycle uses issue → run).
  One validated cutoff flows through selection and final CAS; nil heartbeats
  fall back to run creation time, equality/young exact sources defer, and the
  threshold is not fingerprint input. Due reservations remain atomic one-row
  `FOR UPDATE SKIP LOCKED` transactions. Immutable policy snapshots accept only
  contextually exact partial repeats, claimed supersession closes its attempt
  and lease atomically, and heartbeat retry children keep the locked run status.
  Terminal denial/cancellation/expiry remains scoped to case → approval and
  does not mutate the already-blocked issue. Crash cleanup remains exact-run-
  first and Operations deduplicates its orphan/waiting overlap. Ordinary
  dispatch-failure and wake-delivery lineage, cluster-wide owner epochs,
  durable PubSub/outbox delivery, measured low-resource RAM/CPU/VPS evidence,
  managed workspace-service adoption, and full/latest Paperclip parity remain
  open.
...
- [ ] **L5 managed operator lifecycle — in progress.** Installer rerun guards,
  exact-revision readiness/build identity, read-only service status/logs, and
  exact-revision cutover/rollback form a foundation. Managed install/onboard,
  update, backup/restore, and repair remain open.
