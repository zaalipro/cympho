...
| L3b | P0 | Open | **Durable bounded stranded-work recovery.** Paperclip persists retry lineage and source-state fingerprints, bounds automatic retries, and turns exhaustion into an explicit board-visible recovery action instead of repeating an untracked takeover (`cb0009b097ca05edc0b9ba8f726bcf97f3542408`; `f572e08678d57de7afcf3dcb7788f792190a3291`; `packages/db/src/schema/agent_wakeup_requests.ts`; `server/src/services/recovery/service.ts`). Cympho can fail orphaned runs and return orphaned `in_progress` issues to `todo`, but that reclaim path does not persist recovery attempt lineage/fingerprint or escalate an exhausted series to the board (`lib/cympho/orchestrator/dispatcher.ex`; `lib/cympho/heartbeat_engine/watchdog.ex`). | Persist source-state fingerprint, parent/retry lineage, attempt count, and next retry time across restarts; cap retries deterministically; deduplicate recovery for unchanged state; and create one auditable board escalation when the bounded policy is exhausted. |
...
- [ ] **L3a managed workspace-service ownership — open.** Persist and validate
  ...
  restart.
- [ ] **L3b durable bounded stranded-work recovery — open.** Persist recovery
  lineage and source-state fingerprints, cap retries across restarts, and
  escalate exhaustion once to the board.
