...
Status: active; eleven defined gaps closed and two explicit gaps remain open
Created: 2026-07-30
Last hardened: 2026-08-06 (evidence re-verified against code + focused suites)
...
| G6 | P1 | Remote sandbox provider lifecycle — **open; Fake driver + Runtime wiring only**. | `EnvironmentDriver` behaviour, `EnvironmentDrivers` registry (Fake only; unknown providers fail closed), `EnvironmentLifecycle` acquire/reuse/release/cancel, lease wiring, secret redaction, and Runtime `ensure_provider_environment/1` are implemented and tested. No E2B/Daytona/Cloudflare/Modal/Kubernetes (or other paid/remote) driver is registered. `mix cympho.compare` still reports `remote_sandbox_execution` as a gap. | Define a provider driver contract, registry, lifecycle, cancellation/cleanup semantics, secret boundaries, and one real opt-in provider after a fake driver proves the contract. | Acquire -> execute -> reuse -> release tests, idempotent cleanup, timeout/cancel/orphan recovery, redaction, tenant isolation green for Fake (`test/cympho/workspaces/environment_driver_test.exs`, `environment_lifecycle_test.exs`, Runtime path in `test/cympho/runtime_test.exs`). Real opt-in provider smoke remains open. |
...
| G13 | P2 | Selective standard package workflow — **open; PortablePackage shell only**. | `Cympho.Companies.PortablePackage` exposes selective `:includes` filtering, path/json `load_source/2` with fail-closed path checks, and a collision-mode list. V1 writers still implement whole-company create with `:suffix`/`:fail` slug strategies; `:skip`/`:replace`/`:rename` remain reserved shell modes. No GitHub/ref-pinned remote source and no documented portable directory package format beyond V1 JSON. Comparator may score the facade shell as parity — do not treat that alone as G13 closed. | After G9, add selective includes, dry-run merge, rename/skip/replace, local/GitHub/ref sources, and a documented portable directory format with secret/path scrubbing. | Selective export/include tests in `test/cympho/companies_portability_test.exs` cover the shell. Remaining: every collision writer, ref pinning, disabled timers on import, malicious path/archive matrix, and documented directory format. |
...
- [ ] G6: add one real opt-in remote provider (E2B, Daytona, Cloudflare, Modal, Kubernetes, or equivalent); do not add multiple shallow integrations at once.
...
- [ ] G13: implement skip/replace/rename writers, dry-run merge against an existing company, GitHub/ref-pinned sources, and a documented portable directory format with secret/path scrubbing.
...
## Open residuals (2026-08-06)

Only these register gaps remain open after evidence hardening:

1. **G6** — ship one real opt-in remote environment provider behind the existing driver contract (Fake + Runtime wiring is not enough).
2. **G13** — finish selective merge writers, remote/ref package sources, and a documented portable directory format beyond the PortablePackage shell.

Re-verify with:

```bash
mix cympho.compare --strict
mix test test/cympho/workspaces/environment_driver_test.exs \
