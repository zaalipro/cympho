...
| L3b | P0 | **Foundation delivered; dispatch/wake retry residual open** | **Durable bounded stranded-work recovery.** Cympho now persists company-scoped recovery cases and durable attempt rows with versioned, redacted source fingerprints, source snapshots, parent/root lineage, leases, bounded attempt counts, backoff deadlines, and an active-source uniqueness fence. Dispatcher and Watchdog route both supported source families — `heartbeat_run` and `issue_checkout` — through the durable facade (`priv/repo/migrations/20260903000000_create_recovery_cases.exs`; `lib/cympho/recovery.ex`; `lib/cympho/recovery/recovery_case.ex`; `lib/cympho/recovery/recovery_attempt.ex`; `lib/cympho/recovery/fingerprint.ex`; `lib/cympho/orchestrator/dispatcher.ex`; `lib/cympho/heartbeat_engine/watchdog.ex`). The default policy makes one immediate attempt followed by bounded retries (three attempts total, with 60/120-second delays); unchanged sources are deduplicated as a no-op; stale or cross-tenant/mismatched sources fail closed and are superseded without mutation. On exhaustion, a non-terminal issue is moved to `blocked` and exactly one auditable `stranded_work_recovery` board approval is created. Only an explicitly approved `Retry` reopens the still-matching issue to `todo` and creates a child case; denial, expiry, or cancellation resolves the case while leaving the issue blocked (`test/cympho/recovery_test.exs`; `test/cympho/recovery/fingerprint_test.exs`; `test/cympho/board_approvals/recovery_action_test.exs`; `test/cympho/heartbeat_engine/watchdog_test.exs`; `test/cympho/orchestrator/dispatcher_test.exs`; `test/cympho/owner_attention_test.exs`). This is a foundation, not Paperclip parity: downstream dispatch/wake retry lineage and restart/low-resource evidence remain residuals. | Deliver durable dispatch/wake retry lineage and the documented exit evidence before closing L3b; do not claim Paperclip feature parity, low-resource parity, or restart guarantees beyond the focused tests and source-level contracts above. |
...
- [ ] **L3a managed workspace-service ownership — open.** Persist and validate
  ...
  restart.
- [ ] **L3b durable bounded stranded-work recovery — foundation delivered;
  dispatch/wake retry residual open.** Recovery cases and attempts persist
  versioned fingerprints, bounded leases/retries, and parent/root lineage for
  `heartbeat_run` and `issue_checkout` sources. Exhaustion blocks the issue and
  creates one `stranded_work_recovery` board approval; only an explicitly
  approved `Retry` reopens matching work, while denial/expiry/cancellation
  leaves it blocked. See the migration/context and focused coverage linked in
  the L3b register row. Downstream dispatch/wake retry lineage, restart
  behavior, and low-resource/Paperclip parity evidence remain open.
